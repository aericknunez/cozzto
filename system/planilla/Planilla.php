      ...
      if($a->num_rows > 0){
        ...
        foreach ($a as $b) {
          echo '<tr>
                      ...
                      <td><a id="aplicar" hash="'.$b["hash"].'" op="308"><i class="fa fa-check fa-lg cyan-text"></i></a></td>
                    </tr>';
        }
        echo '</tbody>
        </table>';
...
// sueldo ganado
        echo '<tr>
              <th>Sueldo</th>
              <td>'.Helpers::Dinero($this->SueldoEmpleado($empleado)).'</td>
            </tr>';
// adelantos otorgados
...
/// obtener nombre de empleado
  ...
  public function SumarExtras($tipo, $inicio, $fin){
    $db = new dbConn();
      $a = $db->query("SELECT sum(cantidad) FROM planilla_extras WHERE tipo = '".$tipo."' and fechaF BETWEEN '$inicio' AND '$fin' and td = ".$_SESSION["td"]."");
        foreach ($a as $b) {
...
if($tipo != 1) $sig = "- "; else $sig = "  ";
        ...
        $a = $db->query("SELECT * FROM planilla_extras WHERE empleado='".$empleado."' and tipo='".$tipo."' and td = ".$_SESSION["td"]." order by id desc");
...
$extras = $this->SumarExtras(1, Fechas::Format($data["fecha1_submit"]), Fechas::Format($data["fecha2_submit"])) + $this->SumarExtras(2, Fechas::Format($data["fecha1_submit"]), Fechas::Format($data["fecha2_submit"]));
$descuento = $this->SumarExtras(3, Fechas::Format($data["fecha1_submit"]), Fechas::Format($data["fecha2_submit"]));
...
public function VerDescuentosAsig(){
     $db = new dbConn();
          $a = $db->query("SELECT * FROM planilla_descuentos_asig WHERE td = ".$_SESSION["td"]." order by id desc");
          if($a->num_rows > 0){
        echo '<table class="table table-sm table-hover">
          <thead>
            <tr>
